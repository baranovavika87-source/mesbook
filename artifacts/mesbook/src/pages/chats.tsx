...
export default function ChatsPage() {
  ...
  // МЕРДЖИМ СЕРВЕРНЫЕ ЧАТЫ И ЛОКАЛЬНЫЕ СОХРАНЕНКИ ДЛЯ ДИНАМИЧЕСКОЙ СОРТИРОВКИ
  const allDynamicChats = [...chats];
  const hasSavedInServer = allDynamicChats.some(c => String(c.id) === 'saved');
  if (!hasSavedInServer && savedMessages.length > 0) {
  ...
  const filteredChats = Array.isArray(allDynamicChats) ? allDynamicChats.filter((c: any) => 
  ...
  const handleCreateGroupOrChannel = async () => {
    ...
    setGroupName(''); setChannelName(''); setChannelDesc(''); setModalAvatarUrl(''); setModalType(null); setIsSidebarOpen(false);
  };
  ...
  const handleAddAccountSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = isLoginMode ? { username: newUsername, password: newPassword } : { username: newUsername, password: newPassword, displayName: newName };
      ...
      if (res.ok) {
        ...
        setNewUsername(''); setNewPassword(''); setNewName(''); setShowAddAccountModal(false);
        window.location.reload();
      } else {
        const err = await res.json(); alert(err.error || "Ошибка при входе");
      }
    } catch (e) { alert("Ошибка сети"); }
    setIsAddingAccount(false);
  };

  const switchAccount = (acc: any) => { localStorage.setItem('mesbook_user', JSON.stringify(acc)); window.location.reload(); };
  ...
  const renderChatCard = (chat: any) => {
    ...
    return (
      <Link key={'/chat/' + chat.id} href={'/chat/' + chat.id}>
        <a className="flex items-center px-5 py-3 hover:bg-gray-50/50 dark:hover:bg-zinc-800/50 transition-colors border-b border-gray-200/50 dark:border-zinc-800/80 bg-white dark:bg-[#1c1c1e]">
  ...
  const renderGlobalUserCard = (user: any) => (
    <Link key={user.id} href={'/chat/' + user.id}>
      <a onClick={() => sessionStorage.setItem('chat_name_' + user.id, user.displayName)} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50/50 dark:hover:bg-zinc-800/50 transition-colors border-b border-gray-200/50 dark:border-zinc-800/80 bg-white dark:bg-[#1c1c1e]">
        <div className="flex items-center gap-4">
          <div className="w-[52px] h-[52px] rounded-full bg-gray-100 dark:bg-zinc-800 flex items-center justify-center overflow-hidden shadow-sm border border-gray-200/50 dark:border-zinc-700/50">
            {user.avatarUrl && user.avatarUrl.length > 5 ? <img src={user.avatarUrl} alt="Avatar" className="w-full h-full object-cover" /> : <span className="text-black dark:text-white font-medium text-[20px]">{user.displayName?.charAt(0) || "U"}</span>}
          </div>
  ...
  return (
    <div className="flex h-screen flex-col bg-[#f2f2f7] dark:bg-black transition-colors duration-300 relative overflow-hidden font-sans" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      
      {/* САЙДБАР */}
      {isSidebarOpen && <div className="fixed inset-0 bg-black/40 z-40 transition-opacity backdrop-blur-sm" onClick={() => setIsSidebarOpen(false)} />}
      <div className={`fixed top-0 left-0 h-full w-[85%] max-w-[320px] bg-[#f2f2f7] dark:bg-black z-50 transform transition-transform duration-300 ease-out flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 pb-4 flex justify-between items-start relative bg-white dark:bg-[#1c1c1e] shadow-sm">
          <div className="flex flex-col">
            <div className="w-[60px] h-[60px] bg-gray-100 dark:bg-zinc-800 rounded-full flex items-center justify-center text-[22px] font-bold text-black dark:text-white mb-3 overflow-hidden shadow-sm border border-gray-200/50 dark:border-zinc-700/50">
              {currentUser?.avatarUrl && currentUser.avatarUrl.length > 5 ? <img src={currentUser.avatarUrl} alt="Avatar" className="w-full h-full object-cover" /> : currentUser?.displayName ? currentUser.displayName.charAt(0).toUpperCase() : "U"}
            </div>
            <h2 className="text-[17px] font-semibold text-black dark:text-white leading-tight">{currentUser?.displayName || 'Игорь'}</h2>
            <p className="text-[13px] text-gray-500 dark:text-zinc-400 mt-0.5">{currentUser?.username || '@игорь'}</p>
          </div>
          <button onClick={toggleTheme} className="p-2.5 rounded-full bg-gray-100 dark:bg-zinc-800 text-black dark:text-white transition-colors active:scale-95 border border-gray-200/50 dark:border-zinc-700/50">
        ...
        <div className="flex flex-col py-3 overflow-y-auto flex-1 gap-4 px-3">
          <div className="bg-white dark:bg-[#1c1c1e] rounded-2xl overflow-hidden shadow-sm border border-gray-100/50 dark:border-zinc-800/50">
            {sortedAccounts.length > 1 && (
              <div className="flex flex-col">
                {sortedAccounts.map(acc => {
                  ...
                  return (
                    <button key={acc.id} onClick={() => !isActive && switchAccount(acc)} className={`flex items-center gap-3 px-4 py-3 transition-colors w-full text-left border-b border-gray-100/50 dark:border-zinc-800/50 last:border-0 ${isActive ? 'cursor-default' : 'active:bg-gray-50 dark:active:bg-zinc-800'}`}>
                      ...
                      <span className={`text-[15px] font-medium flex-1 truncate ${isActive ? 'text-black dark:text-white' : 'text-gray-500 dark:text-zinc-400'}`}>{acc.displayName}</span>
            ...
            <button onClick={() => setShowAddAccountModal(true)} className="flex items-center gap-4 px-4 py-3 text-black dark:text-white active:bg-gray-50 dark:active:bg-zinc-800 transition-colors w-full text-left">
          ...
          <div className="bg-white dark:bg-[#1c1c1e] rounded-2xl overflow-hidden shadow-sm flex flex-col border border-gray-100/50 dark:border-zinc-800/50">
            <button onClick={() => setModalType('group')} className="flex items-center gap-4 px-4 py-3 text-black dark:text-white active:bg-gray-50 dark:active:bg-zinc-800 transition-colors w-full text-left border-b border-gray-100/50 dark:border-zinc-800/50">
              <Users size={20} className="text-gray-400 dark:text-zinc-400" />
              <span className="text-[15px] font-medium">Создать группу</span>
            </button>
            <button onClick={() => setModalType('channel')} className="flex items-center gap-4 px-4 py-3 text-black dark:text-white active:bg-gray-50 dark:active:bg-zinc-800 transition-colors w-full text-left">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-[#f2f2f7] dark:bg-black rounded-3xl w-full max-w-sm p-6 shadow-2xl border border-gray-200/50 dark:border-zinc-800/50">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-black dark:text-white">{isLoginMode ? 'Войти в аккаунт' : 'Новый аккаунт'}</h3>
              <button onClick={() => setShowAddAccountModal(false)} className="p-1 text-gray-400 hover:text-black dark:hover:text-white rounded-full transition-colors"><X size={20} /></button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-[#f2f2f7] dark:bg-black flex flex-col animate-in slide-in-from-right duration-200">
          <header className="flex items-center justify-between px-4 pt-10 pb-4 bg-white dark:bg-[#1c1c1e] shadow-sm">
            <div className="flex items-center gap-6">
              <button onClick={() => setModalType(null)} className="text-black dark:text-white transition-colors"><ArrowLeft size={26} strokeWidth={2} /></button>
              <h2 className="text-[20px] font-semibold text-black dark:text-white">{modalType === 'group' ? 'Создать группу' : 'Создать канал'}</h2>
            </div>
            <button onClick={handleCreateGroupOrChannel} disabled={modalType === 'group' ? !groupName.trim() : !channelName.trim()} className="p-1 text-black dark:text-white disabled:opacity-30 transition-opacity"><Check size={26} strokeWidth={2.5} /></button>
          </header>
          <div className="mt-6 px-4">
            <div className="bg-white dark:bg-[#1c1c1e] rounded-3xl p-4 flex items-center gap-4 shadow-sm relative overflow-hidden border border-gray-100/50 dark:border-zinc-800/50">
              <div className="w-[64px] h-[64px] rounded-full bg-[#f2f2f7] dark:bg-black flex items-center justify-center shrink-0 overflow-hidden relative border border-gray-200/50 dark:border-zinc-800 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                {modalAvatarUrl ? <img src={modalAvatarUrl} className="w-full h-full object-cover" /> : <Camera size={28} className="text-gray-400 dark:text-zinc-500" />}
                {isUploadingModalAvatar && <div className="absolute inset-0 bg-black/50 flex items-center justify-center"><Loader2 size={20} className="text-white animate-spin" /></div>}
              </div>
              <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleAvatarUpload} />
              <div className="flex-1">
                <input autoFocus type="text" placeholder={modalType === 'group' ? 'Название группы' : 'Название канала'} value={modalType === 'group' ? groupName : channelName} onChange={e => modalType === 'group' ? setGroupName(e.target.value) : setChannelName(e.target.value)} className="w-full bg-transparent border-b border-gray-200 dark:border-zinc-800 py-2.5 text-[17px] font-medium text-black dark:text-white placeholder-gray-400 outline-none focus:border-black dark:focus:border-white transition-colors" />
              </div>
            </div>
            {modalType === 'channel' && (
              <div className="mt-4 bg-white dark:bg-[#1c1c1e] rounded-3xl shadow-sm overflow-hidden p-4 border border-gray-100/50 dark:border-zinc-800/50">
                ...
                <textarea rows={3} value={channelDesc} onChange={e => setChannelDesc(e.target.value)} className="w-full bg-transparent text-[15px] text-black dark:text-white outline-none resize-none placeholder-gray-400" placeholder="Можете указать дополнительное описание канала." />
              </div>
            )}
            {modalType === 'group' && (
              <div className="mt-6 bg-white dark:bg-[#1c1c1e] rounded-3xl shadow-sm overflow-hidden border border-gray-100/50 dark:border-zinc-800/50">
                <div className="px-4 py-3 border-b border-gray-100 dark:border-zinc-900 bg-gray-50/50 dark:bg-black/20"><span className="text-[13px] font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">1 участник</span></div>
                <div className="flex items-center gap-4 px-4 py-3">
                  <div className="w-12 h-12 rounded-full bg-gray-200 dark:bg-zinc-800 flex items-center justify-center font-bold text-black dark:text-white text-lg overflow-hidden border border-gray-300/30 dark:border-zinc-700/50">
                     {currentUser?.avatarUrl && currentUser.avatarUrl.length > 5 ? <img src={currentUser.avatarUrl} className="w-full h-full object-cover" /> : currentUser?.displayName?.charAt(0).toUpperCase()}
                  </div>
                  <div>
        ...
        <div className="flex flex-col h-full bg-[#f2f2f7] dark:bg-black">
          <header className="px-4 pt-12 pb-0 bg-white dark:bg-[#1c1c1e] relative z-10 flex flex-col shadow-sm border-b border-gray-200/50 dark:border-zinc-900/50">
            <div className="flex items-center gap-4 h-10 mb-3">
              <button onClick={() => { setIsSearchOpen(false); setSearch(''); }} className="text-black dark:text-white active:scale-95 transition-transform"><ArrowLeft size={24} /></button>
              <input autoFocus type="text" placeholder="Поиск" value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 bg-transparent border-none outline-none text-[18px] text-black dark:text-white placeholder-gray-400" />
            </div>
            <div className="flex">
              <button onClick={() => setSearchTab('chats')} className={`flex-1 pb-3 text-[15px] font-semibold transition-colors border-b-[2.5px] ${searchTab === 'chats' ? 'border-black dark:border-white text-black dark:text-white' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-zinc-400'}`}>Чаты</button>
              <button onClick={() => setSearchTab('channels')} className={`flex-1 pb-3 text-[15px] font-semibold transition-colors border-b-[2.5px] ${searchTab === 'channels' ? 'border-black dark:border-white text-black dark:text-white' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-zinc-400'}`}>Каналы</button>
            </div>
          </header>
          <main className="flex-1 overflow-y-auto pt-4 px-4 pb-20">
            {search.length < 2 && searchResults.length === 0 && (
              <div className="text-center py-20 text-gray-400"><Search size={40} className="mx-auto mb-3 opacity-20" /><p className="text-sm">Начните вводить имя или название</p></div>
            )}
            {searchTab === 'chats' ? (
              <div className="flex flex-col gap-4">
                {searchUsersGlobal.length > 0 && (
                  <div className="bg-white dark:bg-[#1c1c1e] rounded-3xl shadow-sm overflow-hidden flex flex-col border border-gray-100/50 dark:border-zinc-800/50">
                    <div className="px-5 py-2.5 border-b border-gray-100/50 dark:border-zinc-800/50 bg-gray-50/50 dark:bg-black/20"><span className="text-[12px] font-bold text-gray-400 uppercase tracking-wider">Глобальный поиск</span></div>
                    {searchUsersGlobal.map(renderGlobalUserCard)}
                  </div>
                )}
                {localChatsFiltered.length > 0 && (
                  <div className="bg-white dark:bg-[#1c1c1e] rounded-3xl shadow-sm overflow-hidden flex flex-col border border-gray-100/50 dark:border-zinc-800/50">
                    <div className="px-5 py-2.5 border-b border-gray-100/50 dark:border-zinc-800/50 bg-gray-50/50 dark:bg-black/20"><span className="text-[12px] font-bold text-gray-400 uppercase tracking-wider">Ваши чаты</span></div>
              ...
              <div className="flex flex-col gap-4">
                ...
                {localChannelsFiltered.length > 0 && (
                  <div className="bg-white dark:bg-[#1c1c1e] rounded-3xl shadow-sm overflow-hidden flex flex-col border border-gray-100/50 dark:border-zinc-800/50">
                    <div className="px-5 py-2.5 border-b border-gray-100/50 dark:border-zinc-800/50 bg-gray-50/50 dark:bg-black/20"><span className="text-[12px] font-bold text-gray-400 uppercase tracking-wider">Ваши каналы</span></div>
        ...
        <>
          <header className="px-6 pt-12 pb-4 relative z-10 bg-[#f2f2f7] dark:bg-black">
            <div className="flex justify-between items-center h-full">
              <button onClick={() => setIsSidebarOpen(true)} className="w-9 h-9 shrink-0 rounded-full bg-gray-200 dark:bg-zinc-800 flex items-center justify-center overflow-hidden border border-gray-300/30 dark:border-zinc-700/50 shadow-sm">
                {currentUser?.avatarUrl && currentUser.avatarUrl.length > 5 ? <img src={currentUser.avatarUrl} alt="Avatar" className="w-full h-full object-cover" /> : <span className="text-black dark:text-white font-medium text-sm">{currentUser?.displayName ? currentUser.displayName.charAt(0).toUpperCase() : "U"}</span>}
              </button>
              <h1 className="text-[20px] font-semibold text-black dark:text-white tracking-wide">Чаты</h1>
              <button onClick={() => setIsSearchOpen(true)} className="w-9 h-9 shrink-0 flex items-center justify-center text-black dark:text-white active:scale-95 transition-transform"><Search size={22} /></button>
            </div>
          </header>
          ...
          <main className="flex-1 overflow-y-auto px-4 pb-4">
            ...
            {filteredChats.length === 0 && (
              <div className="text-center py-20 text-gray-400"><MessageSquare size={48} className="mx-auto mb-3 opacity-20" /><p>Нет чатов</p></div>
            )}
          </main>
          ...
          <nav className="border-t border-gray-200/50 dark:border-zinc-800/50 flex justify-around p-3 bg-white/80 dark:bg-[#1c1c1e]/80 backdrop-blur-md z-10 pb-6">
            <Link href="/"><a className="flex flex-col items-center text-black dark:text-white transition-transform active:scale-95"><MessageSquare size={26} className="mb-1" fill="currentColor" /><span className="text-[10px] font-medium">Чаты</span></a></Link>
            <Link href="/wall"><a className="flex flex-col items-center text-gray-400 hover:text-black dark:hover:text-white transition-colors active:scale-95"><Users size={26} className="mb-1" /><span className="text-[10px] font-medium">Стена</span></a></Link>
          </nav>
        </>
      )}
    </div>
