...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<any[]>(() => {
    try {
      if (isSavedChat) return JSON.parse(localStorage.getItem('mesbook_saved_messages_' + currentUserId) || '[]');
  ...
  const joinChat = async () => {
    try {
      await fetch(`/api/chats/${chatId}/join`, {
        method: 'POST', headers: { 'Authorization': 'Bearer ' + currentUserId }
      });
      setIsMember(true);
      loadData();
    } catch (e) {}
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (err: any) { alert("Ошибка при загрузке файла"); } finally {
  ...
  const handleDelete = async (msgId: number) => {
    ...
    try {
      await fetch('/api/chats/' + chatId + '/messages/' + msgId, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + currentUserId } });
      loadData();
    } catch (e) {}
  };
      ...
      <div className="p-4 bg-white dark:bg-black border-t border-gray-100 dark:border-zinc-900 pb-8 relative z-10 flex flex-col">
        {replyingTo && (
          <div className="flex items-center justify-between mb-3 px-4 py-2 bg-gray-50 dark:bg-zinc-900 rounded-md border-l-2 border-black dark:border-white animate-in slide-in-from-bottom-2 duration-200">
            <div className="flex flex-col overflow-hidden mr-4">
              ...
              <span className="text-[12px] text-gray-500 dark:text-zinc-400 truncate">{replyingTo.content.startsWith('[MEDIA]') ? 'Вложение' : replyingTo.content.replace(/^> .*\n\n/, '')}</span>
            </div>
            <button type="button" onClick={() => setReplyingTo(null)} className="p-1.5 flex-shrink-0 text-gray-400 hover:text-black dark:hover:text-white rounded-full transition-colors"><X size={16} /></button>
          </div>
        )}
        ...
        {!isMember ? (
          ...
          <form onSubmit={handleSend} className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="w-10 h-10 shrink-0 flex items-center justify-center text-gray-400 hover:text-black dark:hover:text-white transition-colors disabled:opacity-50">
              {isUploading ? <Loader2 size={20} className="animate-spin" /> : <Paperclip size={22} />}
            </button>
            <input className="flex-1 bg-gray-100 dark:bg-zinc-900 border-none rounded-md px-4 py-3 outline-none text-black dark:text-white placeholder-gray-400 dark:placeholder-zinc-500 focus:ring-2 focus:ring-black dark:focus:ring-white transition-all ml-1 text-[13px]" value={content} onChange={(e) => setContent(e.target.value)} placeholder="Сообщение..." />
            <button type="submit" disabled={!content.trim()} className="w-11 h-11 flex-shrink-0 rounded-md bg-black dark:bg-white text-white dark:text-black flex items-center justify-center disabled:opacity-40 transition-all active:scale-95 ml-1"><Send size={16} className="ml-1" /></button>
          </form>
        )}
      </div>
    </div>
